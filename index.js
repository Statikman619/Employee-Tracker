...
function modifyWhat() {
  inquirer
    ...
    .then(function (modifyWhatAnswers) {
      if (
        modifyWhatAnswers.modifyWhatChoice === "Change an employee's manager"
      ) {
        modifyMgrEmplSel();
      } else {
        modifyRoleEmplSel();
    ...
    .then(function (newDeptResults) {
      db.query(
        ...
        function (err, res) {
          if (err) throw err;
          console.log(
            `SUCCESS!  You have added the department ${newDeptResults.addDeptAnswer}`
          );
          firstQ();
        }
      );
  ...
  db.query("SELECT * FROM department", function (err, res) {
    ...
    inquirer
      ...
      .then(function (newRoleResults) {
        if (
          newRoleResults.addRoleSalary != parseInt(newRoleResults.addRoleSalary)
        ) {
          console.log(
            `The salary must be numbers only, without letters or special characters.  Please try again.`
              .red
          );
          addRole();
        } else {
          db.query(
            ...
            function (err, res) {
              if (err) throw err;
              console.log(
                `Success!  You have added the role ${newRoleResults.addRoleTitle}`
              );
              firstQ();
            }
          );
...
function addEmployee() {
  db.query("SELECT * FROM role", function (err, res) {
    if (err) throw err;
    db.query("SELECT * FROM employee", function (err1, res1) {
      ...
      for (let i = 0; i < res1.length; i++) {
        choiceArrayManagers.push(
          `${res1[i].id} | ${res1[i].first_name} ${res1[i].last_name}`
        );
      }
      choiceArrayManagers.push("0 | None");
      inquirer
        .prompt([
          {
            type: "input",
          ...
          },
          {
            ...
            choices: choiceArrayRoles,
          },
          {
            type: "list",
            message: "Great, will this employee report to a manager?",
            name: "addEmployeeHasMgr",
            choices: choiceArrayManagers,
          },
        ])
        .then(function (newEmployeeResults) {
          console.log(newEmployeeResults);
          var man = parseInt(
            newEmployeeResults.addEmployeeHasMgr.split(" ")[0]
          );
          if (man === 0) {
            man = null;
          }
          let query = db.query(
            ...
            function (err, res) {
              ...
              if (newEmployeeResults.addEmployeeHasMgr === true) {
                console.log(
                  `Almost there with ${newEmployeeResults.addEmployeeNameF} ${newEmployeeResults.addEmployeeNameL}, just a couple of questions about their manager`
                );
                getMgr();
              } else {
                console.log(
                  `SUCCESS!  You have added ${newEmployeeResults.addEmployeeNameF} ${newEmployeeResults.addEmployeeNameL} to the team!`
                );
                firstQ();
              }
            }
    ...
    inquirer
      ...
      .then(function (mgrQ) {
        ...
        db.query("SELECT id FROM employee", function (err, ans) {
          ...
          if (newest === mgr) {
            console.log(
              `Looks like you have the same id as the employee and the manager.  Please try again.`
            );
            getMgr();
          } else {
            addMgr(newest, mgr);
